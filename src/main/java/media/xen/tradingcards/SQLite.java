    ...
    public Connection getSQLConnection() {
        ...
        if (!folder.exists()) {
            try {
                ...
                TradingCards.getInstance().getLogger().log(Level.SEVERE, "File write error: " + dbname + ".db");
            }
        }
        try {
            ...
            TradingCards.getInstance().getLogger().log(Level.SEVERE, "SQLite exception on initialize", ex);
        } catch (ClassNotFoundException ex) {
            TradingCards.getInstance().getLogger().log(Level.SEVERE,
