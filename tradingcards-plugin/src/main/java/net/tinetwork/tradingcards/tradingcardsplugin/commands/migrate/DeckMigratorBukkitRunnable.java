...
public class DeckMigratorBukkitRunnable extends MigratorBukkitRunnable {
    ...
    public void onExecute() throws ConfigurateException {
        ...
        LoggerUtil.logAndMessage(sender,"Found " + yamlDecks.size() + " players.");

        int totalDecks = yamlDecks.values().stream()
                .mapToInt(Collection::size)
                .sum();

        LoggerUtil.logAndMessage(sender, "Total " + totalDecks + " decks.");

        for (Map.Entry<UUID, List<Deck>> entry : yamlDecks.entrySet()) {
            final UUID playerUuid = entry.getKey();
            LoggerUtil.logAndMessage(sender, InternalMessages.STARTED_CONVERSION_FOR.formatted(playerUuid));
            for (Deck deck : entry.getValue()) {
                plugin.getStorage().saveDeck(playerUuid, deck.getNumber(), deck);
            }
            LoggerUtil.logAndMessage(sender,"&2Finished conversion for " + playerUuid + ", converted " + entry.getValue().size() + " decks.");
        }
        LoggerUtil.logAndMessage(sender, "&2Finished conversion of " + totalDecks + " decks.");
    }
}
